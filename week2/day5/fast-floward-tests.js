...
describe('Flow Dapp Tests', async () => {
    ...
    describe('Fast Floward Tests', async () => {
        ...
        it(`removes a kittyitem from sale`, async () => {
            ...
            let testData2 = { marketCollectionAddress:config.accounts[1] }
            // 3) add testData3 to call kittyItemsMarketReadSaleCollectionIDs
            let testData3 = { marketCollectionAddress:config.accounts[1] }
            ...
            assert.equal(res1.result,1,"User's market collection should have one item")
            // 8) assert the result (an array) from step 6) shows the SaleCollection has 
            // an itemID == 1 in it at index 0
            assert.equal(res2.result[0],1,"User's market collection should have a item type id 1 item")
        })
        ...
        it(`checks both admin and user have correct balances after purchase`, async () => {
            ...
            let testData1 = {address:config.accounts[0]}
            // 2) add testData2 to call kibbleGetBalance for config.accounts[1]
            let testData2 = {address:config.accounts[1]}
            ...
            assert.equal(res1.result, 5.0 , "Incorrect balnace in Account 0 it should be 5.0")
            // 6) assert the result from step 4) equals 25.0
            assert.equal(res2.result,25.0,"Incorrect balnace in Account 1 it should be 25.0")
        })
        it(`delete one kitty item`, async () => {
            let testData = {
                signer: config.accounts[0],
                itemID: "1",
            }
            try {
                await DappLib.BurnKittyItem(testData)
            } catch (e) {
                console.log(e)
            }
        })
